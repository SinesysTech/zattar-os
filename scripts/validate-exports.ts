 ...
 *
 * Funcionalidades:
 * - Escaneia barrels em src/features/*/, src/features/*/*/, src/lib/*/
 ...
 */
function extractExports(sourceFile: ts.SourceFile, program: ts.Program): ExportedSymbol[] {
  ...
  function visit(node: ts.Node) {
    ...
    // export * as namespace from './module'
      ...
      const modulePath = node.moduleSpecifier ? (node.moduleSpecifier as ts.StringLiteral).text : undefined;
 ...
 */
function resolveModulePath(fromFile: string, modulePath: string): string | null {
...
function collectAllExports(
  ...
  for (const exp of exports) {
    if (exp.kind === 'star-reexport' && exp.originalSource) {
      ...
      if (resolvedPath) {
        const subExports = collectAllExports(resolvedPath, program, visited, depth + 1);
...
function collectExportsWithRegex(
  ...
  lines.forEach((line, index) => {
    ...
    if (starMatch && !line.includes(' as ')) {
      ...
      if (resolvedPath && !visited.has(path.resolve(resolvedPath)) && depth < 10) {
        visited.add(path.resolve(resolvedPath));
        const subExports = collectExportsWithRegex(resolvedPath, visited, depth + 1);
        subExports.forEach((symbols, name) => {
          if (!result.has(name)) {
            result.set(name, []);
          }
    ...
    // export { name1, name2 } from './module'
    const namedMatch = line.match(/export\s+(?:type\s+)?\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]/);
    if (namedMatch) {
    ...
    const directMatch = line.match(/export\s+(?:default\s+)?(?:async\s+)?(?:function|const|let|var|class|interface|type|enum)\s+(\w+)/);
    if (directMatch) {
      const name = line.includes('export default') ? 'default' : directMatch[1];
      const kind = line.includes('interface') || line.includes('type ') ? 'type' : 'named';
      if (!result.has(name)) {
        result.set(name, []);
      }
      result.get(name)!.push({
...
function analyzeBarrel(filePath: string, program: ts.Program): BarrelAnalysis {
  ...
  if (sourceFile) {
    ...
    // Fallback para regex
    const content = fs.readFileSync(absolutePath, 'utf-8');
    const tempSourceFile = ts.createSourceFile(absolutePath, content, ts.ScriptTarget.Latest, true);
  ...
  const resolvedExports = collectAllExports(absolutePath, program, new Set(), 0);
  ...
  resolvedExports.forEach((symbols, name) => {
    if (symbols.length > 1) {
      ...
      const uniqueSources = new Set(symbols.map(s => s.originalSource || s.sourceFile));
...
async function validateAllBarrels(): Promise<ValidationReport> {
  ...
  for (const barrel of uniqueBarrels) {
    ...
    if (analysis.duplicates.size > 0) {
      analysis.duplicates.forEach((symbols, symbolName) => {
        const locations = symbols.map(s => {
          const relPath = path.relative(ROOT_DIR, s.sourceFile).replace(/\\/g, '/');
    ...
    log(`  ${analysis.resolvedExports.size} exports, ${analysis.starReexports.length} star re-exports, ${analysis.duplicates.size} duplicatas`);
  }
...
function printReport(report: ValidationReport): void {
  ...
  console.log('\n' + '='.repeat(70));
  console.log(' VALIDACAO DE EXPORTS - SINESYS');
