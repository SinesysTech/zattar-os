...
/**
 * Script: Test Integration Configuration
 * 
 * Testa se as integrações estão configuradas corretamente no banco de dados
 * 
 * Uso:
...
async function testListIntegrations() {
  ...
  if (!data || data.length === 0) {
    ...
    console.log('   Execute: tsx scripts/migrate-integrations-to-db.ts\n');
    return true;
  }

  console.log(`✅ ${data.length} integração(ões) encontrada(s):\n`);
  
  data.forEach((integracao: any) => {
...
async function test2FAuthConfig() {
  ...
  if (error) {
    if (error.code === 'PGRST116') {
      ...
      // Verificar se existe nas variáveis de ambiente
...
async function testDifyConfig() {
  console.log('📋 Teste 4: Buscar configuração do Dify...');
  ...
  if (error) {
    if (error.code === 'PGRST116') {
      console.log('⚠️  Nenhuma integração Dify ativa encontrada');
      
      // Verificar se existe nas variáveis de ambiente
      const hasEnvVars = process.env.DIFY_API_URL && process.env.DIFY_API_KEY;
      if (hasEnvVars) {
        console.log('   ✅ Fallback: Variáveis de ambiente configuradas');
  ...
  console.log('✅ Configuração Dify encontrada:');
  console.log(`   Nome: ${data.nome}`);
  console.log(`   URL: ${data.configuracao.api_url}`);
  console.log(`   API Key: ${data.configuracao.api_key ? '***' + data.configuracao.api_key.slice(-4) : 'não configurado'}`);
  console.log('');

  return true;
}

async function testZapierConfig() {
  console.log('📋 Teste 5: Buscar configuração do Zapier...');
  ...
  if (error) {
    if (error.code === 'PGRST116') {
      console.log('⚠️  Nenhuma integração Zapier ativa encontrada');
      
      // Verificar se existe nas variáveis de ambiente
      const hasEnvVars = process.env.ZAPIER_WEBHOOK_URL;
      if (hasEnvVars) {
        console.log('   ✅ Fallback: Variáveis de ambiente configuradas');
        console.log(`      Webhook: ${process.env.ZAPIER_WEBHOOK_URL}`);
      } else {
        console.log('   ❌ Variáveis de ambiente também não configuradas');
      }
      console.log('');
  ...
  console.log('✅ Configuração Zapier encontrada:');
  console.log(`   Nome: ${data.nome}`);
  console.log(`   Webhook: ${data.configuracao.webhook_url}`);
  console.log('');
...
async function runTests() {
  ...
  const results = {
    ...
    dify: await testDifyConfig(),
    zapier: await testZapierConfig(),
  };
  ...
  if (passed === total) {
    ...
    console.log('\n💡 Possíveis soluções:');
    console.log('   1. Aplicar migration: npx supabase db push');
    console.log('   2. Migrar configurações: tsx scripts/migrate-integrations-to-db.ts');
    console.log('   3. Configurar via UI: /app/configuracoes?tab=integracoes');
  }
