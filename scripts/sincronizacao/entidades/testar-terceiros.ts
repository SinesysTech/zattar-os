    ...
    if (tipoEntidade === 'cliente') {
      ...
      if (temDocumentoValido) {
        ...
        if (!config.dryRun) {
          ...
          console.log(`  🔍 Criando terceiro sem doc: ${parte.nome} (tipo_parte: ${parte.tipo}, polo: ${parte.polo})`);
          ...
          if (result.sucesso && result.terceiro) {
            ...
            entidadeCriada = true;
            resultado.entidadesCriadas.terceiros++;
            console.log(`  ✅ Terceiro criado com ID: ${entidadeId}`);
          } else {
            console.log(`  ❌ Erro ao criar terceiro: ${result.erro}`);
          }
        } else if (config.dryRun) {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Filtro base - APENAS documentos com TERCEIROS
    const filtro: Record<string, unknown> = {
      tipo_captura: 'partes',
      status: 'success',
      'payload_bruto.TERCEIROS.0': { $exists: true }
    };
    console.log('🎯 Filtrando APENAS documentos com TERCEIROS');
    if (config.trt) {
