// Script de teste para captura de Processos Arquivados via API HTTP
 ...
 */
const RESULTS_DIR = join(__dirname, 'results', 'api-arquivados');
 ...
 */
async function testarArquivadosTRT(trtCodigo: CodigoTRT) {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  
  try {
    console.log(`\n🚀 Testando API HTTP - Processos Arquivados - ${trtCodigo}\n`);
...
async function main() {
  ...
  console.log('\n🚀 Iniciando testes de API HTTP - Processos Arquivados\n');
  ...
  for (let i = 0; i < TODOS_TRTS.length; i++) {
    ...
    const resultado = await testarArquivadosTRT(trtCodigo);
