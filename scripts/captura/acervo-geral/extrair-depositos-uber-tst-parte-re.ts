 ...
 * dos processos onde TRABALHADOR recorreu ao TST (Uber é parte ré)
 *
 * Aqui a Uber pode ter depósito recursal de 1º grau (se perdeu em 1ª instância)
 *
 * Uso:
 *   npx tsx --conditions react-server scripts/captura/acervo-geral/extrair-depositos-uber-tst-parte-re.ts
 */
...
const RESULTS_DIR = join(__dirname, '..', '..', 'results', 'depositos-uber-tst-parte-re');
const PDFS_DIR = join(RESULTS_DIR, 'pdfs');
const CREDENCIAL_ID = 49;
const DELAY_ENTRE_DOWNLOADS = 300;
...
async function buscarDocumentosDeposito(): Promise<DocDeposito[]> {
  ...
  // Processos onde Uber é PARTE RÉ no TST (trabalhador recorreu)
  const { data: processos, error } = await supabase
    .from('acervo')
    .select('id, id_pje, numero_processo, nome_parte_autora, classe_judicial, timeline_jsonb')
    .eq('trt', 'TST')
    .eq('origem', 'acervo_geral')
    .ilike('nome_parte_re', '%uber do brasil%')
  ...
  console.log(`📋 ${processos.length} processos onde trabalhador recorreu (Uber parte ré)`);
    ...
    for (const item of timeline) {
      ...
      if (isDeposito) {
        ...
        docs.push({
          ...
          nome_parte_autora: proc.nome_parte_autora as string,
          classe_judicial: proc.classe_judicial as string,
          titulo, tipo, instancia,
  ...
  console.log(`📄 ${deduped.length} documentos únicos de depósito`);
  return deduped;
}
...
async function main() {
  ...
  console.log('\n' + '='.repeat(80));
  console.log('💰 DEPÓSITOS RECURSAIS — UBER PARTE RÉ (TRABALHADOR RECORREU)');
  console.log('='.repeat(80) + '\n');

  try {
    console.log('📋 Fase 1: Buscando documentos de depósito...');
    const docs = await buscarDocumentosDeposito();

    if (docs.length === 0) {
      console.log('ℹ️ Nenhum documento de depósito encontrado');
      return;
    }

    console.log('\n🔐 Fase 2: Autenticando no PJE TST...');
    const credenciais = await buscarCredenciais();
    const tribunalConfig = await getTribunalConfig('TST', 'tribunal_superior');
    const credential: CredenciaisTRT = { cpf: credenciais.cpf, senha: credenciais.senha };

    authResult = await autenticarPJE({ credential, config: tribunalConfig, headless: true });
    ...
    for (let i = 0; i < docs.length; i++) {
      ...
      try {
        const buffer = await baixarDocumento(page, String(doc.id_pje), String(doc.doc_id), {
          incluirCapa: false, incluirAssinatura: false, grau: doc.grau_numerico,
        });
        ...
        resultados.push({
          doc_id: doc.doc_id, numero_processo: doc.numero_processo,
          nome_parte_autora: doc.nome_parte_autora, classe_judicial: doc.classe_judicial,
          titulo: doc.titulo, tipo: doc.tipo, instancia: doc.instancia,
          data_doc: doc.data_doc, pdf_path: pdfFilename, pdf_size: buffer.length, erro: null,
        });
        downloadOk++;
      } catch (e) {
        resultados.push({
          ...
          nome_parte_autora: doc.nome_parte_autora, classe_judicial: doc.classe_judicial,
          titulo: doc.titulo, tipo: doc.tipo, instancia: doc.instancia,
          data_doc: doc.data_doc, pdf_path: null, pdf_size: 0,
    ...
    await writeFile(join(RESULTS_DIR, 'mapeamento-docs.json'), JSON.stringify(resultados, null, 2), 'utf-8');
    ...
    console.log(`   OK: ${downloadOk} | Erro: ${downloadErro}`);
...
main()
  .then(async () => { await new Promise(r => setTimeout(r, 500)); process.exit(0); })
  .catch(async (error) => { console.error('❌', error); await new Promise(r => setTimeout(r, 500)); process.exit(1); });
