...
# 1. Backup Postgres
if docker ps | grep "sinesys_postgres" > /dev/null; then
    echo "Backing up Postgres..."
    docker exec sinesys_postgres pg_dumpall -U postgres > "$BACKUP_PATH/postgres_dump.sql"
...
if docker ps | grep "sinesys_redis" > /dev/null; then
    echo "Backing up Redis dump..."
    # Redis typically saves to /data/dump.rdb. We can copy it out.
    # Force a save first
    docker exec sinesys_redis redis-cli save
    docker cp sinesys_redis:/data/dump.rdb "$BACKUP_PATH/redis_dump.rdb"
else
