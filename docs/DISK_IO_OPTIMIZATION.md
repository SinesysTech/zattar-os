...
- **Funcionalidades**:
  - Cache Hit Rate (índices e tabelas)
  - Disk IO Budget via Management API
  - Queries lentas (>1s)
  - Sequential scans
  - Bloat de tabelas
  - Índices não utilizados
...
- **URL**: `/app/admin/metricas-db/avaliar-upgrade`
- **Assistente**: Análise automatizada de necessidade de upgrade
- **Decisão**: Documentada automaticamente neste arquivo
- **Configuração**: Requer `SUPABASE_PROJECT_REF` e `SUPABASE_ACCESS_TOKEN`

### Alertas Automáticos
- **Cron**: A cada 1 hora (via Supabase Edge Function)
- **Notificações**: Via `criar_notificacao` para super_admins
- **Thresholds**:
  - Cache hit rate < 95%
  - Disk IO Budget > 90% (crítico)
...
> **Instruções**: Use o assistente de decisão em `/app/admin/metricas-db/avaliar-upgrade` para análise automatizada
...
### Recomendação
[PREENCHER após usar assistente de decisão]

### Justificativa
[PREENCHER]

### Custo-Benefício (2026)
| Compute | RAM | IOPS | Throughput | Custo/mês | Recomendado para |
|---------|-----|------|------------|-----------|------------------|
| Micro   | 1GB | 2,085 | 87 MB/s   | $0        | Desenvolvimento  |
| Small   | 2GB | 2,085 | 87 MB/s   | $10       | Produção leve    |
| Medium  | 4GB | 2,085 | 87 MB/s   | $50       | Produção média   |
| Large   | 8GB | 2,085 | 87 MB/s   | $100      | Produção pesada  |
| XL      | 16GB | 3,000 | 125 MB/s  | $200      | Produção alta    |
| 2XL     | 32GB | 3,000 | 125 MB/s  | $400      | Produção crítica |
