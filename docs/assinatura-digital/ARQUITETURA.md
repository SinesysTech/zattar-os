...
```
1. Admin acessa /assinatura-digital/formularios
...
5. POST /api/assinatura-digital/formularios
   ├─ Valida permissões
   ├─ Valida slug único
   ├─ Cria registro em assinatura_digital_formularios
   └─ Retorna formulario_id
6. Admin clica em "Editar Schema"
7. Redireciona para /formularios/[id]/schema
8. FormSchemaBuilder carrega
   ├─ GET /api/assinatura-digital/formularios/[id]
...
10. Admin salva schema
    └─ PUT /api/assinatura-digital/formularios/[id]/schema
...
4. FormularioContainer gerencia steps:
   ...
   │   ├─ Cria sessão em assinatura_digital_assinaturas
   │   ├─ Para cada template:
...
```typescript
// Exemplo: GET /api/assinatura-digital/templates
export async function GET(request: NextRequest) {
  const user = await requirePermission('assinatura_digital', 'listar');
  // ... lógica
}
```

**Permissões:**
- `assinatura_digital.listar` - Listar recursos
- `assinatura_digital.visualizar` - Ver detalhes
- `assinatura_digital.criar` - Criar novos
- `assinatura_digital.editar` - Modificar existentes
- `assinatura_digital.deletar` - Remover
...
**DELETE /api/assinatura-digital/templates/[id]**
...
**GET /api/assinatura-digital/formularios**
...
**POST /api/assinatura-digital/formularios**
...
**PUT /api/assinatura-digital/formularios/[id]**
- Body: Partial<FormsignFormulario>
- Retorna: `{ data: FormsignFormulario }`
- Permissão: `editar`

**DELETE /api/assinatura-digital/formularios/[id]**
- Retorna: `{ success: true }`
- Permissão: `deletar`

**GET /api/assinatura-digital/formularios/[id]/schema**
...
**GET /api/assinatura-digital/segmentos**
...
**GET /api/assinatura-digital/segmentos/[id]**
- Retorna: `{ data: FormsignSegmento }`
- Permissão: `visualizar`
...
- Lógica:
  ...
  2. Cria sessão em `assinatura_digital_assinaturas`
  3. Para cada template:
     - Monta `PdfDataContext` completo
     - Gera PDF final com assinatura
     - Upload para Supabase Storage (`assinatura-digital-pdfs/`)
...
```sql
CREATE TABLE assinatura_digital_templates (
...
CREATE INDEX idx_templates_uuid ON assinatura_digital_templates(template_uuid);
CREATE INDEX idx_templates_status ON assinatura_digital_templates(status);
CREATE INDEX idx_templates_ativo ON assinatura_digital_templates(ativo);
```
...
```sql
CREATE TABLE assinatura_digital_formularios (
  id SERIAL PRIMARY KEY,
  formulario_uuid UUID UNIQUE NOT NULL DEFAULT gen_random_uuid(),
  nome VARCHAR(255) NOT NULL,
  slug VARCHAR(255) NOT NULL,
  segmento_id INTEGER NOT NULL REFERENCES assinatura_digital_segmentos(id),
...
CREATE INDEX idx_formularios_uuid ON assinatura_digital_formularios(formulario_uuid);
CREATE INDEX idx_formularios_segmento ON assinatura_digital_formularios(segmento_id);
CREATE INDEX idx_formularios_slug ON assinatura_digital_formularios(slug);
CREATE INDEX idx_formularios_ativo ON assinatura_digital_formularios(ativo);
```
...
```sql
CREATE TABLE assinatura_digital_segmentos (
  id SERIAL PRIMARY KEY,
  nome VARCHAR(255) NOT NULL,
...
CREATE INDEX idx_segmentos_slug ON assinatura_digital_segmentos(slug);
CREATE INDEX idx_segmentos_ativo ON assinatura_digital_segmentos(ativo);
```

#### assinatura_digital_assinaturas

```sql
CREATE TABLE assinatura_digital_assinaturas (
  id SERIAL PRIMARY KEY,
  sessao_uuid UUID UNIQUE NOT NULL DEFAULT gen_random_uuid(),
  formulario_id INTEGER NOT NULL REFERENCES assinatura_digital_formularios(id),
...
CREATE INDEX idx_sessoes_uuid ON assinatura_digital_assinaturas(sessao_uuid);
CREATE INDEX idx_sessoes_formulario ON assinatura_digital_assinaturas(formulario_id);
CREATE INDEX idx_sessoes_cliente ON assinatura_digital_assinaturas(cliente_id);
CREATE INDEX idx_sessoes_criado_em ON assinatura_digital_assinaturas(criado_em);
```
...
```sql
-- Tabela já existente no Sinesys, usada pelo Assinatura Digital
...
**Bucket: `assinatura-digital-pdfs`**
...
- Componentes pesados carregados dinamicamente:
  ```typescript
  const FieldMappingEditor = dynamic(() => import('@/components/assinatura-digital/editor/FieldMappingEditor'));
  const PdfPreviewDynamic = dynamic(() => import('@/components/assinatura-digital/pdf/PdfPreviewDynamic'));
  ```
