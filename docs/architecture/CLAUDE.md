...
Sinesys (Zattar OS) is a legal management system (gestão jurídica) for a Brazilian law firm. The codebase is primarily in Portuguese (variable names, business terms, UI labels).
...
# Build
npm run build                        # Standard build (~3.5min)
...
- Auto-redirects configured: `/{module}` → `/app/{module}` for all 35+ modules

### Middleware (`middleware.ts`)

Handles Supabase session management, CSP nonce generation, IP blocking/whitelisting, and multi-app routing.
...
- No vertical dividers between columns (`hideColumnBorders: true` default)
- Column alignment default: `left` (set via `meta.align`)
...
- SQL style: lowercase keywords, snake_case identifiers, plural table names, singular column names
...
- **Partes**: Clients, opposing parties, third parties, representatives (CPF/CNPJ validation)
...
- **Financeiro**: Dashboard, accounts payable/receivable, OFX/CSV bank reconciliation (special subdomain pattern)
