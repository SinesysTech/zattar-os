...
- Erro 403 ao acessar `/assinatura-digital/admin`
...
**Soluções:**
1. Verifique se você tem permissão `formsign_admin.listar`
...
```sql
-- Segmentos
SELECT * FROM formsign_segmentos WHERE slug = 'seu-slug';

-- Formulários
SELECT * FROM formsign_formularios WHERE slug = 'seu-slug';
```
...
**Soluções:**
1. Verifique se bucket `formsign-pdfs` existe
2. Verifique políticas RLS:
   ```sql
   -- Templates: leitura pública
   CREATE POLICY "Public read templates"
   ON storage.objects FOR SELECT
   USING (bucket_id = 'formsign-pdfs' AND (storage.foldername(name))[1] = 'templates');

   -- Sessões: leitura com sessao_uuid
   CREATE POLICY "Read sessoes with uuid"
   ON storage.objects FOR SELECT
   USING (bucket_id = 'formsign-pdfs' AND (storage.foldername(name))[1] = 'sessoes');
   ```
...
SELECT 
  ...
  f.template_ids,
  f.form_schema
FROM formsign_formularios f
JOIN formsign_segmentos s ON f.segmento_id = s.id
...
SELECT 
  ...
  s.ip_address,
  s.user_agent
FROM formsign_sessoes s
JOIN formsign_formularios f ON s.formulario_id = f.id
