...
2. **Docker Swarm Mode** ativado (o Portainer detecta automaticamente)
3. **Traefik** configurado e rodando como serviço no Swarm
4. **Rede Docker** chamada `network_swarm_public` criada (ou ajuste o nome no docker-compose.yml)
5. **Domínio** configurado apontando para o servidor
6. **Certificado SSL** configurado no Traefik (Let's Encrypt recomendado)
7. **Imagem Docker buildada** antes do deploy (Docker Swarm não suporta `build:` diretamente)
...
⚠️ **IMPORTANTE**: Docker Swarm **não suporta** `build:` diretamente no docker-compose.yml. Você **deve** fazer o build da imagem **antes** de fazer o deploy!

### 1. Preparar o Build da Imagem (OBRIGATÓRIO)

Você tem algumas opções:

#### Opção A: Build no Portainer (Recomendado)
...
# Se você tem múltiplos nós no Swarm, faça push para um registry:
...
#### Opção C: Build via Registry Docker

Se você tem um registry Docker configurado:
...
⚠️ **IMPORTANTE**: O arquivo `docker-compose.yml` está configurado para **Docker Swarm Mode** e usa `image:` ao invés de `build:`. Isso significa que você **deve** fazer o build da imagem antes (veja passo 1).

O arquivo está estruturado assim:

```yaml
services:
  zattar_advogados:
    image: zattar_advogados:latest  # Imagem já buildada
    deploy:
      mode: replicated
      replicas: 1
...
**Diferenças do Docker Swarm:**
- ✅ Usa `deploy:` com `mode:`, `replicas:`, `restart_policy:`, etc.
- ✅ Labels do Traefik ficam dentro de `deploy.labels:`
- ❌ **NÃO** usa `build:` (faça build antes)
- ❌ **NÃO** usa `restart:` direto (use `restart_policy:` dentro de `deploy:`)
- ❌ **NÃO** usa `container_name:` (Swarm gerencia automaticamente)
