...
import { DEFAULT_ZOOM_CONFIG, PDF_CANVAS_SIZE } from '@/types/formsign/pdf-preview.types';
import type { Template, TemplateCampo, ApiPreviewTestResponse } from '@/types/formsign/template.types';
...
interface FieldMappingEditorProps {
  template: Template; // from @/types/formsign/template.types
  onCancel?: () => void;
  mode?: 'edit' | 'create';
}
...
export default function FieldMappingEditor({ template, onCancel, mode = 'edit' }: FieldMappingEditorProps) {
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      ...
      const originalUrl = `/api/assinatura-digital/admin/templates/${template.id}/preview`;
  ...
  const handleCancel = useCallback(() => {
    if (hasUnsavedChanges) {
      setPendingNavigation(
        onCancel ? () => onCancel() : () => router.push('/assinatura-digital/admin/templates'),
      );
      setShowExitConfirmation(true);
    } else if (onCancel) {
      onCancel();
    } else {
      router.push('/assinatura-digital/admin/templates');
    }
  ...
  const handleGenerateTestPreview = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/assinatura-digital/admin/templates/${template.id}/preview-test`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleTemplateInfoUpdate = useCallback(async (updates: Partial<Template>) => {
    try {
      if (mode === 'create') {
        ...
        setPdfUrl(`/api/assinatura-digital/admin/templates/${newTemplate.id}/preview`);
        setPreviewKey((prev) => prev + 1);

        // Redirecionar para a página de edição do template criado
        router.replace(`/assinatura-digital/admin/templates/${newTemplate.id}/edit`);
  ...
  const handleReplacePdfSuccess = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/assinatura-digital/admin/templates/${templateId}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      const newPdfUrl = `/api/assinatura-digital/admin/templates/${templateId}/preview?t=${Date.now()}`;
