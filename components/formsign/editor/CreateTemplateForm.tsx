...
export default function CreateTemplateForm({
  ...
  const canSubmit = formData.nome.trim() !== '';
    ...
    try {
      const dataToSubmit = {
        ...
        ...(formData.conteudo_markdown.trim() && {
          conteudo_markdown: formData.conteudo_markdown.trim()
        }),
      };
          ...
          {formData.conteudo_markdown.trim() === '' ? (
            <div className="border-2 border-dashed rounded-lg p-6 text-center space-y-2">
              <p className="text-sm text-muted-foreground">
                Nenhum conteúdo Markdown configurado
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="border rounded-lg p-4 space-y-3 bg-muted/20">
              <div className="prose prose-sm max-w-none line-clamp-5">
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  rehypePlugins={[rehypeRaw, rehypeSanitize]}
                >
                  {formData.conteudo_markdown}
                </ReactMarkdown>
              </div>
              <Button
                variant="outline"
