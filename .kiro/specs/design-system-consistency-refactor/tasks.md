...
Migração incremental de 5 módulos para o Design System ZattarOS, usando o módulo **partes** como padrão ouro. A ordem de execução é: dependência compartilhada (variants.ts) → polimento do padrão ouro → migração módulo a módulo → testes de validação.
