...
CREATE POLICY "Usuários podem ver seus próprios arquivos"
  ...
  USING (criado_por IN (SELECT id FROM public.usuarios WHERE auth_user_id = (select auth.uid())));

CREATE POLICY "Usuários podem criar arquivos"
  ON public.arquivos FOR INSERT
  WITH CHECK (criado_por IN (SELECT id FROM public.usuarios WHERE auth_user_id = (select auth.uid())));
