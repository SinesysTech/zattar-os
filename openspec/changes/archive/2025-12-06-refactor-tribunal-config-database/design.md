...
O sistema ZattarOS captura dados de 24 Tribunais Regionais do Trabalho (TRT1 a TRT24), cada um com URLs específicas para login, API e base. Atualmente essas configurações estão hardcoded em `backend/captura/services/trt/config.ts` (~400 linhas de código).
