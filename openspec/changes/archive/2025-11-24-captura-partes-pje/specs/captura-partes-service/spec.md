# Spec: Serviço de Captura de Partes

## Capability
`captura-partes-service`

## Purpose
Serviço backend para orquestrar captura de partes de processos do PJE-TRT, processamento de dados, identificação de clientes vs partes contrárias, e persistência no banco de dados.

## Scope
Lógica de negócio para capturar partes, identificar tipo de parte, upsert de entidades, e criação de relacionamentos processo-partes.
...
### Location
- **Files**: `backend/captura/services/partes/`
  - `partes-capture.service.ts` - Função principal de captura
  - `processar-parte.service.ts` - Processar parte individual
  - `processar-representantes.service.ts` - Processar representantes
  - `criar-vinculo.service.ts` - Criar vínculo processo-parte
  - `types.ts` - Tipos de resultado

### Dependencies
- `obterPartesProcesso()` de `backend/api/pje-trt/partes/`
- `obterRepresentantesParte()` de `backend/api/pje-trt/partes/`
- `identificarTipoParte()` de `backend/captura/services/partes/identificacao-partes.service.ts`
...
- `upsertRepresentante()` de `backend/representantes/services/representantes-persistence.service.ts`
- `upsertProcessoParte()` de `backend/partes/services/processo-partes-persistence.service.ts`
- Supabase client para transações

### Result Type
```typescript
interface CapturaPartesResult {
  processoId: number;
...
### Error Handling
- Errors ao buscar partes: Logar e retornar resultado parcial
- Errors ao processar parte: Logar, adicionar a erros[], continuar
- Errors críticos de banco: Rollback transação, lançar erro

## Testing Requirements
- Unit tests: Processar diferentes tipos de partes
- Integration tests: Captura completa de processo real
- Transaction tests: Verificar rollback em erros
- Performance tests: Tempo de captura de 100 processos

## References
- Spec `pje-partes-api` para funções de API
- Spec `identificacao-cliente` para lógica de identificação
- Serviços de persistência existentes em `backend/partes/` e `backend/representantes/`
