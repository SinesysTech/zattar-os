# Spec: API Routes para Captura de Partes

## Capability
`api-routes`

## Purpose
Endpoint REST para acionar captura de partes de processos do PJE-TRT via interface HTTP.

## Scope
Endpoint `POST /api/captura/trt/partes` para iniciar captura de partes de processos específicos ou todos os processos de um advogado.
...
### Location
- **File**: `app/api/captura/trt/partes/route.ts`
...
### Request Body Type
```typescript
interface CapturaPartesRequestBody {
  advogado_id: number;
...
### Response Type
```typescript
interface CapturaPartesResponse {
  success: boolean;
  data?: {
...
### Flow
```typescript
1. Authenticate request → authenticateRequest(request)
2. Parse and validate body → JSON.parse, Zod validation
3. Fetch advogado → buscarAdvogadoPorId(advogado_id)
4. Fetch credenciais → buscarCredenciaisPorIds(credencial_ids)
5. Validate credenciais belong to advogado
6. Fetch processos (if processo_ids provided, else all from advogado)
7. Create captura_log record → status: 'in_progress'
8. For each processo:
   - Call capturarPartesProcesso(processo_id, credencial)
   - Accumulate results
9. Update captura_log → status: 'completed', resultado
10. Return NextResponse.json({ success: true, data })
```
...
try {
  ...
  console.error('[API-CAPTURA-PARTES] Erro', error);

  // Update captura_log if exists
  if (capturaLogId) {
    await atualizarCapturaLog(capturaLogId, {
      status: 'failed',
      erro: error.message
    });
  }
...
- Validate error responses (400, 401, 403, 404)

### Integration Tests
- End-to-end: POST request → capture → database
- Verify captura_log creation and update
- Multiple processos success
- Partial failure handling

### Security Tests
- Unauthorized access (no token)
- Token from different user
- Credenciais from different advogado

## References
- Spec `captura-partes-service` for business logic
- Existing endpoints in `app/api/captura/trt/` for pattern reference
- `authenticateRequest()` from `backend/utils/auth/api-auth.ts`
- `capturas_log` table schema
