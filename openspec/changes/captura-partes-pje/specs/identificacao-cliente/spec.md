# Spec: Identificação de Clientes via CPF de Representantes

## Capability
`identificacao-cliente`

## Purpose
Lógica de negócio para identificar automaticamente quais partes de um processo são nossos clientes baseado no CPF dos advogados representantes comparado com CPF do advogado proprietário da credencial de acesso ao PJE.

## Scope
Algoritmo de classificação de partes em: cliente, parte contrária ou terceiro.
...
### Location
- **File**: `backend/captura/services/partes/identificacao-partes.service.ts`
...
function isTipoEspecial(tipoParte: string): boolean;
function verificarRepresentantesComCpf(
...
### Algorithm
```typescript
1. Check if tipoParte is in TIPOS_ESPECIAIS
   → Return 'terceiro'

2. Get representantes array (default to [])

3. Normalize advogado CPF (remove non-digits)

4. For each representante:
   a. Skip if no CPF
   b. Normalize representante CPF
   c. Compare normalized CPFs
   d. If match → Return 'cliente'

5. No match found → Return 'parte_contraria'
```
...
- [ ] Terceiro: Tipo especial independente de representante
...
### Integration Tests
- [ ] Classificação correta em processo real
- [ ] Múltiplos advogados do escritório
- [ ] Edge cases: Perito representado por nós

### Property-Based Tests
- [ ] Qualquer parte com representante nosso = cliente
- [ ] Tipo especial sempre = terceiro
- [ ] Sem representante nosso = parte_contraria

## References
- Tipos `PartePJE` e `RepresentantePJE` de spec `pje-partes-api`
- Tipo `Advogado` de `backend/types/advogados/advogados-types.ts`
- Lista de tipos especiais baseada em análise de dados reais do PJE
