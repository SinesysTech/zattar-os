# Spec: Sistema de Editor de Documentos
...
### C-DOC-001: Nomenclatura de Arquivos
- Arquivos backend: `kebab-case.service.ts`
- Componentes React: `PascalCase.tsx`
- Hooks customizados: `use-kebab-case.ts`

### C-DOC-002: Estrutura de Diretórios
Seguir padrão do Sinesys:
```
backend/documentos/
  services/
    persistence/
...
### C-DOC-003: RLS Policies
- Sempre granular: uma policy por operação (select, insert, update, delete)
- Sempre especificar role: `TO authenticated`
- Nunca combinar policies mesmo que tenham lógica idêntica

### C-DOC-004: Supabase Realtime
- Usar componentes existentes: `RealtimeCursors`, `RealtimeAvatarStack`, `RealtimeChat`
- Não criar implementação customizada de Realtime

### C-DOC-005: Backblaze B2
- Bucket: `zattar-advogados`
- Prefixo: `editor/`
- Nomenclatura: `editor/doc_{id}/{timestamp}_{random}.{ext}`

### C-DOC-006: Auto-Save
- Debounce: 2 segundos
- Endpoint dedicado: `/api/documentos/[id]/auto-save`
- NÃO incrementar versão
- NÃO criar entrada em `documentos_versoes`

### C-DOC-007: Versionamento
- Salvar versão ANTES de atualizar documento
- Incrementar `versao` apenas em salvamento explícito (Ctrl+S)
- Auto-save NÃO incrementa versão

### C-DOC-008: Soft Delete
- Todas as tabelas relevantes têm `deleted_at`
- Job de limpeza: deletar permanentemente após 30 dias
- Lixeira: listar itens com `deleted_at IS NOT NULL`

### C-DOC-009: Permissões
- NÃO integrar com sistema de 82 permissões
- Permissões a nível de item (documento/pasta)
- Criador define permissões ao compartilhar
- Configurável (não hard-coded em RLS)

### C-DOC-010: Plate.js
- NÃO modificar componentes em `components/plate/`
- Criar wrappers em `components/documentos/`
- Suprimir settings original do Plate.js
- Configurar IA via env vars (Open Router)
