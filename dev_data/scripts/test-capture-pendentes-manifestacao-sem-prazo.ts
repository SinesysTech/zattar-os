// Script de desenvolvimento para testar captura de Pendentes de Manifestação (SEM PRAZO) do TRT
 ...
 */
const RESULTS_DIR = join(__dirname, 'results', 'pendentes-manifestacao', 'sem-prazo');
 ...
 */
async function testarPendentesManifestacaoSemPrazo(config: TestConfig) {
  const logCapture = new LogCapture();
  logCapture.start();

  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');

  console.log('\n🚀 Iniciando teste de captura - Pendentes de Manifestação (SEM PRAZO)\n');
  ...
  try {
    ...
    console.log('\n⏳ Iniciando autenticação e captura de Pendentes de Manifestação (SEM PRAZO)...\n');
...
async function main() {
  ...
  await testarPendentesManifestacaoSemPrazo(testConfig);
}
...
export { testarPendentesManifestacaoSemPrazo, type TestConfig };
