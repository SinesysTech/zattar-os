// Script de desenvolvimento para testar captura de Pendentes de Manifestação do TRT
 ...
 */
const RESULTS_DIR = join(__dirname, 'results', 'pendentes-manifestacao');
 ...
 */
async function testarPendentesManifestacao(config: TestConfig) {
  ...
  try {
    console.log('\n🚀 Iniciando teste de captura - Pendentes de Manifestação\n');
    ...
    console.log('\n⏳ Iniciando autenticação e captura de Pendentes de Manifestação...\n');

    // 5. Executar captura
    const inicio = Date.now();
    const resultado: PendentesManifestacaoResult = await pendentesManifestacaoCapture(params);
    ...
    console.log(`  - Total de processos pendentes: ${resultado.total}`);
...
async function main() {
  ...
  await testarPendentesManifestacao(testConfig);
}
...
export { testarPendentesManifestacao, type TestConfig };
