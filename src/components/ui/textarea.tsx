...
const VARIANT_CLASSES: Record<TextareaVariant, string> = {
  default:
    "border-input bg-transparent px-[var(--density-control-px)] py-[var(--density-control-px)] shadow-xs dark:bg-input/30 rounded-md",
  glass:
...
function Textarea({ className, variant = "default", ...props }: TextareaProps) {
  return (
    <textarea
      ...
      className={cn(
        "flex field-sizing-content min-h-16 w-full border text-base transition-[color,background-color,border-color,box-shadow] outline-none md:text-[length:var(--density-control-font-size)]",
