...
export const createMockSupabaseForIntegration = () => {
  // Tipos inferidos a partir do literal — evita incompatibilidade entre
  // `jest.Mock<T, Y>` (API antiga de @jest/globals@27) e `jest.Mock<T, Y, C>`
  // (API nova de @types/jest@30). Cada método fica com a assinatura real que
  // usa, em vez de `Mock<UnknownFunction>` redundante.
