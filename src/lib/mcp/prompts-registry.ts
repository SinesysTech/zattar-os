/**
 * Registry de Prompts MCP do ZattarOS
 *
...
${argumentos.map((a, i) => `${i + 1}. ${a}`).join("\n")}`;
  ...
  registerMcpPrompt({
    ...
    handler: async (args) => {
      ...
      const systemPrompt = `Você é um assistente jurídico do ZattarOS, especializado em direito trabalhista.
...
5. Recomendações de ação`;
  ...
  registerMcpPrompt({
    ...
    handler: async (args) => {
      ...
      const systemPrompt = `Você é um assistente jurídico virtual do ZattarOS, especializado em direito trabalhista brasileiro.
