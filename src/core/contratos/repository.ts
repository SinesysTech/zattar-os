/**
 * CONTRATOS REPOSITORY - Camada de Persistencia
 *
 * Este arquivo contem funcoes de acesso ao banco de dados para Contratos.
 *
 * CONVENCOES:
...
/**
 * Verifica se uma parte contraria existe
 */
export async function parteContrariaExists(parteContrariaId: number): Promise<Result<boolean>> {
  try {
    ...
    return err(
      appError(
        'DATABASE_ERROR',
        'Erro ao verificar parte contraria',
        undefined,
        error instanceof Error ? error : undefined
      )
    );
...
export async function updateContrato(
  ...
  try {
    ...
    // Preparar dados para atualizacao (snake_case)
