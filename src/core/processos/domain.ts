 ...
 */
export interface ProcessoUnificado extends Omit<Processo, 'id' | 'grau' | 'origem'> {
 ...
 */
export const grauProcessoSchema = z.enum(['primeiro_grau', 'segundo_grau', 'tribunal_superior']);
...
export const createProcessoSchema = z.object({
  ...
  numeroProcesso: z
    ...
    .regex(REGEX_NUMERO_CNJ, 'Numero do processo deve seguir o padrao CNJ (NNNNNNN-DD.AAAA.J.TT.OOOO)'),
  numero: z.number().int().positive('Numero deve ser positivo'),
  descricaoOrgaoJulgador: z.string().min(1, 'Descricao do orgao julgador e obrigatoria'),
  classeJudicial: z.string().min(1, 'Classe judicial e obrigatoria'),
  codigoStatusProcesso: z.string().min(1, 'Codigo do status do processo e obrigatorio'),
...
export const updateProcessoSchema = z.object({
  ...
  advogadoId: z.number().int().positive('ID do advogado deve ser positivo').optional(),
  ...
  descricaoOrgaoJulgador: z.string().min(1, 'Descricao do orgao julgador e obrigatoria').optional(),
  classeJudicial: z.string().min(1, 'Classe judicial e obrigatoria').optional(),
  codigoStatusProcesso: z.string().min(1, 'Codigo do status do processo e obrigatorio').optional(),
  nomeParteAutora: z.string().min(1, 'Nome da parte autora e obrigatorio').optional(),
