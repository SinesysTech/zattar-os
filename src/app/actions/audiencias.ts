...
import { createAudienciaSchema, ListarAudienciasParams, StatusAudiencia, updateAudienciaSchema } from '@/core/audiencias/domain';
...
export async function actionAtualizarStatusAudiencia(
  ...
  const userId = undefined; // Faking user for now

  const result = await audienciasService.atualizarStatusAudiencia(id, status, statusDescricao, userId);
...
export async function actionListarAudiencias(params: ListarAudienciasParams): Promise<ActionResult<any>> {
