...
function normalizarFiltrosPrazo(
  ...
  const candidatos =
    filtros && Array.isArray(filtros)
      ? filtros
      : filtroUnico
        ? [filtroUnico]
        : [];
  const valores: FiltroPrazoPendentes[] =
    candidatos.length > 0 ? candidatos : ["sem_prazo"];
  const invalidos = valores.filter((valor) => !FILTROS_VALIDOS.includes(valor));

  if (invalidos.length > 0) {
    throw new Error(
      `filtroPrazo/filtrosPrazo inválido(s): ${invalidos.join(", ")}`,
    );
  }

  const unicos = Array.from(new Set(valores));
  return unicos.sort(
    (a, b) => FILTROS_VALIDOS.indexOf(a) - FILTROS_VALIDOS.indexOf(b),
  );
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { advogado_id, credencial_ids, filtroPrazo, filtrosPrazo } =
      body as PendentesManifestacaoParams;
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            error instanceof Error ? error.message : "Filtro de prazo inválido",
        },
        { status: 400 },
      );
    }
    ...
    // 5.5. Pré-carregar configurações de tribunais em paralelo (evitar N+1 com cache memory-level)
    const uniqueTribunais = Array.from(
      new Set(credenciaisOrdenadas.map((c) => `${c.tribunal}:${c.grau}`)),
    );
    await Promise.all(
      uniqueTribunais.map(async (key) => {
        const [tribunal, grau] = key.split(":");
        try {
          // Tipagem cast pois getTribunalConfig usa CodeTRT e GrauTRT, e sabemos que as strings vêm da base de credenciais
          await getTribunalConfig(
            tribunal as import("@/app/(authenticated)/captura/types/trt-types").CodigoTRT,
            grau as import("@/app/(authenticated)/captura/types/trt-types").GrauTRT,
          );
        } catch (e) {
          // Erros serão capturados novamente e tratados no loop principal, aqui apenas ignoramos falha do pre-load
          console.error(
            `Erro ao pré-carregar config do tribunal ${tribunal} ${grau}:`,
            e,
          );
        }
      }),
    ...
    (async () => {
      ...
      for (const credCompleta of credenciaisOrdenadas) {
        console.log(
          `[Pendentes] Iniciando captura: ${credCompleta.tribunal} ${credCompleta.grau} (Credencial ID: ${credCompleta.credentialId})`,
        );
        ...
        for (const filtro of filtrosParaExecutar) {
          try {
            ...
            console.log(
              `[Pendentes] Captura concluída (${filtro}): ${credCompleta.tribunal} ${credCompleta.grau} (Credencial ID: ${credCompleta.credentialId})`,
            );
            ...
            console.error(
              `[Pendentes] Erro ao capturar ${credCompleta.tribunal} ${credCompleta.grau} (Credencial ID: ${credCompleta.credentialId}) para filtro ${filtro}:`,
              error,
            );

            const erroMsg =
              error instanceof Error &&
              error.message.includes("Quantidade de processos")
                ? error.message
                : error instanceof Error
                  ? error.message
                  : "Erro desconhecido";
      ...
      if (logId) {
        try {
          const errosColetados = resultados.flatMap((r) => {
            const errosFiltro =
              r.filtros
                ...
                .map(
                  (f) =>
                    `${r.tribunal} ${r.grau} (ID ${r.credencial_id}) - ${f.filtroPrazo}: ${f.erro}`,
                ) || [];
          ...
          console.log(
            `[Pendentes] Processamento concluído. Total: ${resultados.length} credenciais processadas.`,
          );
        } catch (error) {
