...
import { acervoGeralCapture } from '@/features/captura/services/trt/acervo-geral.service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 6. Processar cada credencial SEQUENCIALMENTE (não bloqueia resposta)
    (async () => {
      ...
      for (const credCompleta of credenciaisOrdenadas) {
        ...
        // Executar captura sequencialmente
        try {
          ...
          console.log(`[Acervo Geral] Captura concluída: ${credCompleta.tribunal} ${credCompleta.grau} (Credencial ID: ${credCompleta.credentialId})`);
      ...
      // Atualizar histórico após conclusão de todas as capturas
      if (logId) {
        try {
          ...
          console.log(`[Acervo Geral] Processamento concluído. Total: ${resultados.length} credenciais processadas.`);
        } catch (error) {
          console.error('Erro ao atualizar histórico de captura:', error);
        }
      }
    })().catch((error) => {
      console.error('[Acervo Geral] Erro ao processar capturas:', error);
      if (logId) {
    ...
    // 7. Retornar resultado imediato (resposta assíncrona)
    return NextResponse.json({
      success: true,
