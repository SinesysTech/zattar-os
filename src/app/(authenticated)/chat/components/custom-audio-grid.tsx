...
export function CustomAudioGrid({ className }: CustomAudioGridProps) {
  ...
  return (
    <div className={cn("flex flex-wrap items-center justify-center inline-extra-loose inset-extra-loose h-full relative", className)}>
      ...
      {allParticipants.map((p: DyteParticipant) => (
        <div key={p.id} className={cn("flex flex-col items-center inline-default group")}>
          ...
          <div className="text-center">
            <p className={cn( "font-semibold text-video-text text-body-lg")}>{p.name} {p.id === self?.id && "(Você)"}</p>
