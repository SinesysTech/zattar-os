...
export function AudienciaStatusBadge({ status, className, compact = false }: AudienciaStatusBadgeProps) {
  ...
  return (
    <SemanticBadge category="audiencia_status" value={status} className={cn(/* design-system-escape: text-xs → migrar para <Text variant="caption">; font-medium → className de <Text>/<Heading> */ 'text-xs font-medium', className)}>
