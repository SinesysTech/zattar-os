...
export function EditarAudienciaDialog({ open, onOpenChange, onSuccess, audiencia }: EditarAudienciaDialogProps) {
  ...
  React.useEffect(() => {
    if (open && audiencia) {
      ...
      if (audiencia.dataInicio) {
        ...
        setHoraInicio(audiencia.horaInicio || '');
      }
      if (audiencia.dataFim) {
        const d_fim = new Date(audiencia.dataFim);
      ...
      if (audiencia.enderecoPresencial) {
        ...
        setComplemento(audiencia.enderecoPresencial.complemento || '');
        setBairro(audiencia.enderecoPresencial.bairro || '');
        setCidade(audiencia.enderecoPresencial.cidade || '');
        setEstado(audiencia.enderecoPresencial.uf || '');
        setCep(audiencia.enderecoPresencial.cep || '');
      }
    }
  }, [open, audiencia, formatYYYYMMDD]);
      ...
      if (audiencia && !processosRetornados.find(p => p.id === audiencia.processoId)) {
        processosRetornados = [
          ...processosRetornados, 
          {
            ...
            polo_ativo_nome: audiencia.poloAtivoNome || '',
            polo_passivo_nome: audiencia.poloPassivoNome || '',
            trt: audiencia.trt,
            grau: audiencia.grau,
            orgao_julgador_id: audiencia.orgaoJulgadorId || 0,
          }
        ];
      }
      setProcessos(processosRetornados);
    } catch (err) {
  ...
  React.useEffect(() => {
    if (trt && grau) {
      ...
      setTiposAudiencia([]);
      if (!audiencia) setTipoAudienciaId('');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [trt, grau]);
  ...
  React.useEffect(() => {
    if (processoSelecionado) {
      ...
      setSalas([]);
      if (!audiencia) setSalaAudienciaId('');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [processoSelecionado]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await actionAtualizarAudienciaPayload(audiencia.id, {
        ...
        dataFim: dataFimISO,
        tipoAudienciaId: tipoAudienciaId ? parseInt(tipoAudienciaId) : null,
        salaAudienciaNome: salaAudienciaNome || null,
        urlAudienciaVirtual: urlVirtual || null,
