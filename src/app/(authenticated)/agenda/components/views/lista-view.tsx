...
export function ListaView({ events, onEventClick, className }: ListaViewProps) {
  ...
  return (
    <GlassPanel className={cn("overflow-hidden", className)}>
      <div className={cn("grid grid-cols-[minmax(180px,1.5fr)_minmax(140px,2fr)_100px_100px_90px_40px] inline-default px-4 py-3 border-b border-border/10")}>
      ...
      <div>
        {paged.map((evt) => {
          ...
          return (
            <button key={evt.id} onClick={() => onEventClick?.(evt)} className={cn("w-full grid grid-cols-[minmax(180px,1.5fr)_minmax(140px,2fr)_100px_100px_90px_40px] inline-default items-center px-4 py-3.5 text-left border-b border-border/6 transition-all cursor-pointer hover:bg-muted/6")}>
      ...
      {totalPages > 1 && (
        <div className={cn("flex items-center justify-between px-4 py-3 border-t border-border/10")}>
          ...
          <div className={cn("flex items-center inline-micro")}>
            <button onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0} className={cn("px-2 py-1 rounded-lg text-[10px] text-muted-foreground/40 hover:bg-muted/15 disabled:opacity-30 cursor-pointer disabled:cursor-default transition-colors")}>Anterior</button>
            ...
            <button onClick={() => setPage(Math.min(totalPages - 1, page + 1))} disabled={page >= totalPages - 1} className={cn("px-2 py-1 rounded-lg text-[10px] text-muted-foreground/40 hover:bg-muted/15 disabled:opacity-30 cursor-pointer disabled:cursor-default transition-colors")}>Próximo</button>
          </div>
        </div>
      )}
    </GlassPanel>
