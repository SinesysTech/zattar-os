...
function TypeRow({ token, size, children }: TypeRowProps) {
  return (
    <div className={cn("grid grid-cols-[100px_1fr_90px] items-baseline inline-default border-b border-dashed border-border py-2 last:border-b-0 sm:grid-cols-[120px_1fr_80px]")}>
...
function TypeBody() {
  return (
    ...
    >
      ...
      <div className={cn("mt-4 flex items-baseline inline-default rounded-xl border border-border bg-card px-3.5 py-3")}>
...
function TypeMeta() {
  ...
  return (
    <SpecimenCard eyebrow="LABELS · META · MONO">
      ...
      <div className={cn(/* design-system-escape: p-3.5 → usar <Inset> */ "mt-4 grid grid-cols-2 inline-medium rounded-xl border border-border bg-card p-3.5 sm:grid-cols-4")}>
        {metas.map((m) => (
          <div key={m.label} className={cn("flex flex-col inline-micro")}>
