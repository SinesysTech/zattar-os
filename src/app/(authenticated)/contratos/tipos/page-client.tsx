...
function TipoDialog({ open, onOpenChange, tipo, onSuccess }: TipoDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <Form {...form}>
        <form id="tipo-form" onSubmit={form.handleSubmit(onSubmit)} className={cn("flex flex-col stack-default")}>
          <FormField
      ...
      <GlassPanel depth={1} className="overflow-hidden">
        {isLoading ? (
          ...
          <div role="table" aria-label="Tipos de Contrato">
            <div
              role="row"
              className={cn(/* design-system-escape: gap-4 → migrar para <Inline gap="default">; px-4 padding direcional sem Inset equiv.; py-2.5 padding direcional sem Inset equiv.; tracking-wide sem token DS; */ "text-overline grid grid-cols-[1.5fr_1fr_80px_100px_120px] inline-default px-4 py-2.5 border-b border-border/40 text-muted-foreground/70")}
            >
              <span>Nome</span>
              <span>Slug</span>
              <span>Ordem</span>
