...
export function PagarContaDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (comprovante) {
        // Assume que anexos existing devem ser mantidos ou substituídos?
        // Aqui estamos adicionando ao array existente ou criando novo array
            ...
            <div className={cn(/* design-system-escape: p-4 → migrar para <Inset variant="card-compact"> */ "rounded-lg border bg-muted/50 p-4 stack-tight")}>
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <p className={cn(/* design-system-escape: font-bold → className de <Text>/<Heading> */ "font-bold text-body-lg")}>{formatarValor(conta.valor)}</p>
