...
/**
 * MaisFiltrosPopover - Popover com filtros avançados para Contas a Pagar/Receber
 *
...
interface MaisFiltrosPopoverProps {
  ...
  formaPagamentoOptions: FilterOption[];
...
export function MaisFiltrosPopover({
  tipoRecorrente,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className={cn(/* design-system-escape: p-4 → migrar para <Inset variant="card-compact"> */ "w-80 p-4")} align="start">
        <div className={cn(/* design-system-escape: space-y-4 → migrar para <Stack gap="default"> */ "space-y-4")}>
          ...
          {/* Forma de Pagamento */}
          <div className={cn(/* design-system-escape: space-y-1.5 sem token DS */ "space-y-1.5")}>
            <Label className={cn(/* design-system-escape: text-xs → migrar para <Text variant="caption"> */ "text-xs text-muted-foreground")}>Forma de Pagamento</Label>
            <Select
              value={formaPagamento || 'all'}
              onValueChange={(val) => onFormaPagamentoChange(val === 'all' ? '' : val)}
            >
              <SelectTrigger className="h-9 w-full">
