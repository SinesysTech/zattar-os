...
export function TabToggle({
  ...
  return (
    <div className={cn("flex inline-nano inset-nano rounded-lg bg-border/10")}>
      {tabs.map((tab) => (
        <button
          key={tab.id}
      ...
      {segments.map((seg, i) => (
        ...
        >
          ...
          {(seg.value / total) > 0.12 && (
            <div className={cn("absolute inset-0 flex flex-col items-center justify-center inset-micro")}>
