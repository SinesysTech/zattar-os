    ...
    try {
      ...
      const diag = await page
        .evaluate(() => {
          ...
          const corpoLow = corpoTexto.toLowerCase();
          const ehErroSeam = urlAtual.includes('error.seam');
          const indicaManutencao =
            corpoLow.includes('manutenção') ||
            corpoLow.includes('indisponível') ||
            corpoLow.includes('temporariamente');
          // Bloqueio de WAF/CDN — assinaturas conhecidas
          const ehBloqueioWaf =
          ...
          return { urlAtual, titulo, corpoTexto, ehErroSeam, indicaManutencao, ehBloqueioWaf };
        })
        .catch(() => ({ urlAtual: '', titulo: '', corpoTexto: '', ehErroSeam: false, indicaManutencao: false, ehBloqueioWaf: false }));

      // Bloqueio de WAF/CDN: retry não resolve, falhar imediatamente
      if (diag.ehBloqueioWaf) {
        log('error', '🛡️ Tribunal bloqueando o IP/User-Agent via WAF/CDN', diag);
        throw new Error(
          `Tribunal bloqueando acesso (WAF/CDN). Title: "${diag.titulo}". URL: ${diag.urlAtual}. ` +
            `Causa provável: IP do browser remoto ou User-Agent na lista de bloqueio. ` +
            `Solução: rotacionar User-Agent ou contatar suporte do tribunal para liberar o IP.`,
        );
      }

      // Página de erro do PJE (não-WAF): falhar imediatamente sem retry
