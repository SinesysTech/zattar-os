...
import {
  ...
  CATEGORIAS_CONTA_RECEBER_PADRAO,
  type FormaRecebimentoContaReceber,
...
export function ContaReceberFormDialog({
  ...
  const onSubmit = async (data: ContaReceberFormData) => {
    try {
      ...
      if (isEditMode && conta) {
        const resultado = await atualizarContaReceber(conta.id, payload);
        if (!resultado.success) {
          throw new Error(resultado.error);
        }
        toast.success('Conta atualizada com sucesso!');
      } else {
        const resultado = await criarContaReceber(payload);
        if (!resultado.success) {
          throw new Error(resultado.error);
        }
