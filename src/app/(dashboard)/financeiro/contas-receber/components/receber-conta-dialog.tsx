...
export function ReceberContaDialog({
  ...
  /**
   * Faz upload do comprovante e retorna o objeto AnexoContaReceber
   */
  const uploadComprovante = async (file: File): Promise<AnexoContaReceber | null> => {
    ...
    try {
      ...
      const uniqueName = `comprovantes-recebimento/${timestamp}-${randomId}.${extension}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultado = await receberConta(conta.id, {
        formaRecebimento,
