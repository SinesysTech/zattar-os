...
const FORMULARIOS_FILTER_CONFIGS = [
  { id: 'segmento_id', label: 'Segmento', type: 'multi-select' as const },
  { id: 'ativo', label: 'Ativo', type: 'select' as const, options: [
    { value: 'true', label: 'Sim' },
    { value: 'false', label: 'Não' },
  ] },
  { id: 'foto_necessaria', label: 'Foto necessária', type: 'select' as const, options: [
    { value: 'true', label: 'Sim' },
    { value: 'false', label: 'Não' },
  ] },
  { id: 'geolocation_necessaria', label: 'Geolocalização necessária', type: 'select' as const, options: [
    { value: 'true', label: 'Sim' },
    { value: 'false', label: 'Não' },
  ] },
];
  ...
  for (const config of FORMULARIOS_FILTER_CONFIGS) {
    if (config.type === 'select') {
      ...
      for (const segmento of segmentos) {
        options.push({ value: `${config.id}_${segmento.id}`, label: segmento.nome, group: config.label });
      }
    }
  }
  return options;
...
function buildFormulariosFilterGroups(segmentos: AssinaturaDigitalSegmento[]) {
  return FORMULARIOS_FILTER_CONFIGS.map(config => {
    if (config.type === 'select') {
      return { label: config.label, options: config.options.map(opt => ({ value: `${config.id}_${opt.value}`, label: opt.label })) };
    } else if (config.type === 'multi-select' && config.id === 'segmento_id') {
      return { label: config.label, options: segmentos.map(s => ({ value: `${config.id}_${s.id}`, label: s.nome })) };
    }
...
function criarColunas(onEditSchema: (formulario: AssinaturaDigitalFormulario) => void, onDuplicate: (formulario: AssinaturaDigitalFormulario) => void, onDelete: (formulario: AssinaturaDigitalFormulario) => void, templates: AssinaturaDigitalTemplate[], canEdit: boolean, canCreate: boolean, canDelete: boolean): ColumnDef<AssinaturaDigitalFormulario>[] {
  return [
    ...
    { accessorKey: 'foto_necessaria', header: ({ column }) => (
      <div className="flex items-center justify-center"><DataTableColumnHeader column={column} title="Foto Necessária" /></div>
    ), enableSorting: false, size: 140, cell: ({ row }) => {
      const value = row.getValue('foto_necessaria') as boolean;
      return (
        <div className="min-h-10 flex items-center justify-center">
          <Badge variant={getBooleanBadgeVariant(value)} className="capitalize">{formatBooleanBadge(value)}</Badge>
        </div>
      );
    } },
    { accessorKey: 'geolocation_necessaria', header: ({ column }) => (
      <div className="flex items-center justify-center"><DataTableColumnHeader column={column} title="Geolocalização Necessária" /></div>
    ), enableSorting: false, size: 160, cell: ({ row }) => {
...
function FormularioActions({ formulario, onEditSchema, onDuplicate, onDelete, canEdit, canCreate, canDelete }: { formulario: AssinaturaDigitalFormulario; onEditSchema: (formulario: AssinaturaDigitalFormulario) => void; onDuplicate: (formulario: AssinaturaDigitalFormulario) => void; onDelete: (formulario: AssinaturaDigitalFormulario) => void; canEdit: boolean; canCreate: boolean; canDelete: boolean; }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        {canEdit && (<DropdownMenuItem onClick={() => onEditSchema(formulario)}><FileText className="mr-2 h-4 w-4" />Editar Schema</DropdownMenuItem>)}
        {canCreate && (<DropdownMenuItem onClick={() => onDuplicate(formulario)}><Copy className="mr-2 h-4 w-4" />Duplicar</DropdownMenuItem>)}
        {canDelete && (<DropdownMenuItem onClick={() => onDelete(formulario)} className="text-destructive"><Trash2 className="mr-2 h-4 w-4" />Deletar</DropdownMenuItem>)}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
...
export function FormulariosClient() {
  ...
  const params = React.useMemo(() => ({ pagina: pagina + 1, limite, busca: buscaDebounced || undefined, segmento_id: filtros.segmento_id, ativo: filtros.ativo, foto_necessaria: filtros.foto_necessaria, geolocation_necessaria: filtros.geolocation_necessaria }), [pagina, limite, buscaDebounced, filtros]);
  ...
  const handleFilterIdsChange = React.useCallback((selectedIds: string[]) => { setSelectedFilterIds(selectedIds); const newFilters = parseFormulariosFilters(selectedIds); setFiltros(newFilters); setPagina(0); }, []);
  ...
  const handleDelete = React.useCallback((formulario: AssinaturaDigitalFormulario) => { setSelectedFormularios([formulario]); setDeleteOpen(true); }, []);
  const handleBulkDelete = React.useCallback(() => { const selected = Object.keys(rowSelection).map(id => formularios.find(f => f.id === Number(id))).filter(Boolean) as AssinaturaDigitalFormulario[]; setSelectedFormularios(selected); setDeleteOpen(true); }, [rowSelection, formularios]);
