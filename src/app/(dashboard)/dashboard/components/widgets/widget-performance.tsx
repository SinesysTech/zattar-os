...
// ============================================================================
// Pre-configured Variant: Widget Produtividade Performance
// ============================================================================

interface WidgetProdutividadePerformanceProps {
  data: ProdutividadeResumo;
  className?: string;
}

export function WidgetProdutividadePerformance({
  data,
  className,
}: WidgetProdutividadePerformanceProps) {
  // Dados já vêm calculados do backend
  const { baixasHoje, baixasSemana, baixasMes, mediaDiaria, comparativoSemanaAnterior } = data;
  ...
  const metrics: PerformanceMetric[] = [
    ...
    },
    {
      label: 'Semana',
      value: baixasSemana,
      trend: comparativoSemanaAnterior !== 0 ? Math.abs(comparativoSemanaAnterior) : undefined,
      trendDir: comparativoSemanaAnterior !== 0 ? (comparativoSemanaAnterior >= 0 ? 'up' : 'down') : undefined,
      icon: Calendar,
    },
    {
      label: 'Mês',
  ...
  if (baixasHoje > 0) {
    activities.push({
      id: '1',
      text: `${baixasHoje} expediente${baixasHoje > 1 ? 's' : ''} baixado${baixasHoje > 1 ? 's' : ''} hoje`,
      date: 'Hoje',
      status: 'success',
    });
  }

  if (baixasSemana > baixasHoje) {
    activities.push({
      id: '2',
      text: `${baixasSemana - baixasHoje} expedientes nos últimos 7 dias`,
      date: 'Semana',
      status: 'success',
    });
  }
  ...
  if (progressValue < 60) {
    activities.push({
      id: '3',
      text: `Meta mensal: ${progressValue}% concluída`,
      date: 'Atenção',
      status: 'warning',
    });
    ...
    activities.push({
      id: '3',
      text: `Excelente! Meta ${progressValue >= 100 ? 'atingida' : 'quase atingida'}`,
      date: 'Parabéns',
      status: 'success',
    });
  }
