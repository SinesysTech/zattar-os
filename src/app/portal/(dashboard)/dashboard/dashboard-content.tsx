...
export function DashboardContent({ data, error }: DashboardContentProps) {
  if (error || !data) {
    return (
      <EmptyState
        icon={AlertCircle}
        title={error ?? "Nenhum dado disponivel"}
        description="Tente recarregar a pagina ou entre em contato com o escritorio."
      />
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <PortalSectionHeader
        title="Dashboard"
        description={`Bem-vindo, ${data.cliente.nome}`}
      />

      {/* Partial errors banner */}
      {data.errors && (
        <div className="rounded-md border border-portal-warning/30 bg-portal-warning-soft p-3 text-sm text-portal-warning">
          ...
          <CardContent>
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-lg border bg-muted">
                <span className="text-xs font-bold uppercase text-portal-text-muted">
              ...
              <div className="min-w-0 flex-1 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-portal-text-muted">
              ...
              {processosRecentes.map((processo) => (
                ...
                >
                  <div className="min-w-0 flex-1 space-y-1">
                    ...
                    <p className="text-sm text-portal-text-muted truncate">
                  ...
                  {processo.ultima_movimentacao && (
                    <div className="shrink-0 text-right">
                      <p className="text-xs text-portal-text-muted whitespace-nowrap">
                        {formatDateBR(processo.ultima_movimentacao.data)}
                      </p>
                      <p className="text-xs text-portal-text-muted truncate max-w-50">
