...
export function useAdaptiveQuality(
  ...
  useEffect(() => {
            ...
            poorConnectionTimer.current = setTimeout(() => {
                if (autoSwitch) {
                    ...
                    toastIdRef.current = toast.warning("Conexão lenta detectada", {
                        description: "Desativar vídeo pode melhorar a qualidade do áudio.",
                        action: {
    ...
    return () => {
        if (poorConnectionTimer.current) clearTimeout(poorConnectionTimer.current);
        if (goodConnectionTimer.current) clearTimeout(goodConnectionTimer.current);
    };
  }, [meeting, score, threshold, duration, autoSwitch, isVideoDisabledByAdaptive]);
