...
export function ChatFooter({ salaId, onEnviarMensagem, onTyping, typingIndicatorText }: ChatFooterProps) {
  ...
  return (
    <div className="lg:px-4 pb-4">
      ...
      <div className={`relative flex items-center rounded-md border transition-colors duration-300 ${isRecording ? 'bg-red-50 border-red-200' : 'bg-muted'}`}>
        ...
        {isRecording ? (
          ...
          <div className="flex-1 flex items-center justify-between h-14 px-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-red-600 font-medium">{formatDuration(recordingDuration)}</span>
              <span className="text-xs text-red-400 animate-pulse hidden sm:inline-block">Gravando áudio...</span>
            </div>

            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={cancelRecording} className="text-slate-500 hover:text-red-600 hover:bg-red-100 rounded-full px-4">
                Cancelar
              </Button>
              <Button size="icon" onClick={handleSendAudio} className="rounded-full bg-red-500 hover:bg-red-600 text-white h-9 w-9">
