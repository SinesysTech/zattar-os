...
export type ActionResponse<T = any> = {
  ...
  data?: T;
  error?: string;
  errorDetail?: any; // For structurred errors like CargoComUsuariosError
};
...
export async function actionListarCargos(
  ...
  try {
    ...
    const hasPermission = await checkPermission(user.id, 'cargos:visualizar') || await checkPermission(user.id, 'usuarios:visualizar');
...
export async function actionBuscarCargo(id: number): Promise<ActionResponse> {
  try {
    ...
    const hasPermission = await checkPermission(user.id, 'cargos:visualizar');
    if (!hasPermission) return { success: false, error: 'Sem permissão' };
...
export async function actionCriarCargo(params: CriarCargoDTO): Promise<ActionResponse> {
  try {
    const user = await getCurrentUser();
    if (!user) return { success: false, error: 'Não autenticado' };

    const hasPermission = await checkPermission(user.id, 'cargos:editar') || await checkPermission(user.id, 'usuarios:editar');
    if (!hasPermission) return { success: false, error: 'Sem permissão' };
