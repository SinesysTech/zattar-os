 ...
 */
export async function findPecaModeloById(id: number): Promise<Result<PecaModelo | null>> {
  try {
...
export async function findAllPecasModelos(
  ...
  try {
    ...
    // Ordenação
    const orderColumn =
      orderBy === 'titulo' ? 'titulo' : orderBy === 'uso_count' ? 'uso_count' : 'created_at';
...
export async function updatePecaModelo(
  ...
  try {
    ...
    if (input.visibilidade !== undefined) updateData.visibilidade = input.visibilidade;
    if (input.segmentoId !== undefined) updateData.segmento_id = input.segmentoId;
...
export async function deletePecaModelo(id: number): Promise<Result<void>> {
  try {
    ...
    const { error } = await db.from(TABLE_PECAS_MODELOS).update({ ativo: false }).eq('id', id);
 ...
 */
export async function deleteContratoDocumento(id: number): Promise<Result<void>> {
  try {
    const db = createDbClient();

    const { error } = await db.from(TABLE_CONTRATO_DOCUMENTOS).delete().eq('id', id);
