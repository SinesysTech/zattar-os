...
import type { PoloProcessoParte, TipoParteProcesso } from '@/types/domain/processo-partes';
...
function mapTelefone(entidade: EntidadeContatoRow): { ddd_telefone: string | null; numero_telefone: string | null } {
  // Preferir residencial; fallback para comercial
  const ddd = entidade.ddd_residencial ?? entidade.ddd_comercial ?? null;
  const numero = entidade.numero_residencial ?? entidade.numero_comercial ?? null;
  return { ddd_telefone: ddd, numero_telefone: numero };
}

function mapParteComDados(vinculo: VínculoProcessoParteRow, entidade: EntidadeContatoRow | undefined): ParteComDadosCompletos {
  const telefone = entidade ? mapTelefone(entidade) : { ddd_telefone: null, numero_telefone: null };
...
export async function actionBuscarPartesPorProcessoEPolo(
  processoId: number,
  polo: PoloProcessoParte
): Promise<ActionResult<{ partes: ParteComDadosCompletos[]; principal: ParteComDadosCompletos | null }>> {
  try {
    if (!processoId || processoId <= 0) {
      return { success: false, error: 'processoId inválido', message: 'Parâmetros inválidos.' };
    }

    const polosValidos: PoloProcessoParte[] = ['ATIVO', 'PASSIVO', 'NEUTRO', 'TERCEIRO'];
    if (!polosValidos.includes(polo)) {
      return { success: false, error: 'polo inválido', message: 'Parâmetros inválidos.' };
    }
    ...
    if (vinculosError) {
      return { success: false, error: vinculosError.message, message: 'Falha ao buscar partes do processo.' };
    }

    const vinculos = (vinculosRaw ?? []) as unknown as VínculoProcessoParteRow[];
    if (vinculos.length === 0) {
      return { success: true, data: { partes: [], principal: null }, message: 'Nenhuma parte encontrada.' };
    }

    const idsClientes = vinculos.filter(v => v.tipo_entidade === 'cliente').map(v => v.entidade_id);
    const idsPartesContrarias = vinculos.filter(v => v.tipo_entidade === 'parte_contraria').map(v => v.entidade_id);
    const idsTerceiros = vinculos.filter(v => v.tipo_entidade === 'terceiro').map(v => v.entidade_id);
    ...
    const firstError = clientesRes.error ?? partesContrariasRes.error ?? terceirosRes.error;
    if (firstError) {
      return { success: false, error: firstError.message, message: 'Falha ao buscar dados das partes.' };
    }

    const clientes = (clientesRes.data ?? []) as unknown as EntidadeContatoRow[];
    const partesContrarias = (partesContrariasRes.data ?? []) as unknown as EntidadeContatoRow[];
    const terceiros = (terceirosRes.data ?? []) as unknown as EntidadeContatoRow[];
