...
import { GRAU_TRIBUNAL_LABELS } from '@/core/audiencias/domain';
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-[400px] sm:w-[540px] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pr-4 -mr-4">
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              {audiencia.enderecoPresencial && (
                <p className="ml-7 text-sm text-muted-foreground">
                  ...
                  {audiencia.enderecoPresencial.uf}, CEP: {audiencia.enderecoPresencial.cep}
                </p>
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="ml-7 flex items-center gap-3">
                  <Avatar className="h-9 w-9">
                    <AvatarImage
                      src={`/avatars/${audiencia.responsavelId % 5}.png`}
                      alt="Responsável"
                    />
                    <AvatarFallback>
                      {audiencia.responsavelId.toString().slice(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <span className="font-medium">Usuário {audiencia.responsavelId}</span>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2 text-xs text-muted-foreground">
              <p>
                Criado em:{' '}
                {format(new Date(audiencia.createdAt), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
              </p>
              <p>
                Atualizado em:{' '}
                {format(new Date(audiencia.updatedAt), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2 border-t pt-4">
          ...
          <Button onClick={() => console.log('Edit Audiencia', audiencia.id)}>Editar</Button>
        </div>
      </SheetContent>
    </Sheet>
  );
