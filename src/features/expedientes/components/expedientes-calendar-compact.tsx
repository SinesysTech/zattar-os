...
interface ExpedientesCalendarCompactProps {
  /** Data selecionada (dia) */
  selectedDate: Date;
  /** Callback quando um dia é selecionado */
  onDateSelect: (date: Date) => void;
  /** Expedientes para contar/destacar dias */
...
export function ExpedientesCalendarCompact({
  selectedDate,
  onDateSelect,
  expedientes,
  currentMonth,
  onMonthChange,
  className,
}: ExpedientesCalendarCompactProps) {
  // Gerar células do calendário
  const cells = React.useMemo(() => getCalendarCells(currentMonth), [currentMonth]);

  // Mapa de expedientes por dia (ISO date string -> expedientes[])
  const expedientesByDay = React.useMemo(() => {
    const map = new Map<string, Expediente[]>();
    expedientes.forEach((exp) => {
      // Só inclui expedientes que tenham data de prazo
      if (!exp.dataPrazoLegalParte) return;

      const dateKey = format(parseISO(exp.dataPrazoLegalParte), 'yyyy-MM-dd');
      ...
      <div className="border rounded-md overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          {cells.map((cell, index) => {
            ...
            const dayExpedientes = expedientesByDay.get(dateKey) || [];
            const hasExpedientes = dayExpedientes.length > 0;
