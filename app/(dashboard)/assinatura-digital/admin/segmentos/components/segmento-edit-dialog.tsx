...
import { segmentoSchema } from '@/types/formsign/segmento.types';
import { generateSlug } from '@/app/_lib/formsign/slug-helpers';
import { FormsignSegmento } from '@/backend/types/formsign-admin/types';
...
interface SegmentoEditDialogProps {
  ...
  segmento: FormsignSegmento | null;
  onSuccess: () => void;
}
  ...
  const onSubmit = async (data: EditSegmentoFormData) => {
    ...
    try {
      ...
      if (data.slug !== segmento.slug) {
        const checkResponse = await fetch(`/api/assinatura-digital/admin/segmentos?slug=${encodeURIComponent(data.slug)}`);
        if (!checkResponse.ok) {
      ...
      // Proceed with update
      const response = await fetch(`/api/assinatura-digital/admin/segmentos/${segmento.id}`, {
        method: 'PUT',
        headers: {
