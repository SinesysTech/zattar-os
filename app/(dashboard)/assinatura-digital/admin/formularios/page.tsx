...
// Funções de filtro
function buildFormulariosFilterOptions(segmentos: FormsignSegmento[]) {
  const options = [];
...
function buildFormulariosFilterGroups(segmentos: FormsignSegmento[]) {
...
function useFormularios(params: {
  ...
  const fetchFormularios = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/assinatura-digital/admin/formularios?${searchParams}`);
...
function useSegmentos() {
  ...
  const fetchSegmentos = React.useCallback(async () => {
    setData(prev => ({ ...prev, isLoading: true, error: null }));
    try {
      const res = await fetch('/api/assinatura-digital/admin/segmentos?ativo=true');
      const json = await res.json();
      if (!res.ok || json.error) {
...
function useTemplates() {
  ...
  const fetchTemplates = React.useCallback(async () => {
    setData(prev => ({ ...prev, isLoading: true, error: null }));
    try {
      const res = await fetch('/api/assinatura-digital/admin/templates?ativo=true');
      const json = await res.json();
      if (!res.ok || json.error) {
...
export default function FormulariosPage() {
  ...
  // Handlers para ações
  const handleEditSchema = React.useCallback((formulario: FormsignFormulario) => {
    router.push(`/assinatura-digital/admin/formularios/${formulario.id}/schema`);
  }, [router]);

  const handleDuplicate = React.useCallback((formulario: FormsignFormulario) => {
    setSelectedFormulario(formulario);
    setDuplicateOpen(true);
  }, []);

  const handleDelete = React.useCallback((formulario: FormsignFormulario) => {
    setSelectedFormularios([formulario]);
    setDeleteOpen(true);
  }, []);

  const handleBulkDelete = React.useCallback(() => {
    const selected = Object.keys(rowSelection).map(id => formularios.find(f => f.id === Number(id))).filter(Boolean) as FormsignFormulario[];
  ...
  const handleExportCSV = React.useCallback(() => {
    const selected = Object.keys(rowSelection).length > 0
      ? Object.keys(rowSelection).map(id => formularios.find(f => f.id === Number(id))).filter(Boolean) as FormsignFormulario[]
      : formularios;
