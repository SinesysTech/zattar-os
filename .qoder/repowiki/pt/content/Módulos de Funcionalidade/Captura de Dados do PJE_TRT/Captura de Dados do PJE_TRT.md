...
**Fontes da seção**
- [trt-capture.service.ts](file://backend/captura/services/trt/trt-capture.service.ts#L1-L33)
...
Os scripts de teste em `dev_data/scripts/` validam o funcionamento do módulo. Eles incluem:
...
Os testes simulam chamadas aos serviços, verificam a estrutura dos dados retornados e validam a persistência. Os resultados são salvos em arquivos JSON e logs, permitindo análise detalhada. Os dados de teste em `dev_data/scripts/results/` incluem resultados reais de captura, úteis para depuração e validação de regressão.
