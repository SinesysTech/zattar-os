...
export async function getOTP(config?: TwoFAuthConfig): Promise<OTPResult> {
  // Usar variáveis de ambiente se config não fornecido
  ...
  try {
    ...
    // Tratar resposta de erro (400, 401, 403)
    if (!response.ok) {
    ...
    // Validar que o campo password existe
    if (!data.password) {
      throw new TwoFAuthError(
        500,
        '2FAuth retornou resposta sem campo password',
        { response: JSON.stringify(data) }
      );
    }

    // Validar tamanho do password (6-10 caracteres conforme documentação)
    ...
    // Se já é um TwoFAuthError, re-lançar
    ...
    if (error instanceof Error) {
      throw new TwoFAuthError(
        500,
        `Erro ao chamar API 2FAuth: ${error.message}`,
    ...
    throw new TwoFAuthError(
      500,
      'Erro desconhecido ao chamar API 2FAuth',
      { error: String(error) }
    );
  }
}
